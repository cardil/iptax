...
class TestAuthenticate:
    ...
    async def test_authenticate_with_kerberos_success(
        ...
        # Should have navigated to URL
        mock_page.goto.assert_called_once_with("https://wd5.myworkday.com/example")
        # Should have waited for page load
        mock_page.wait_for_load_state.assert_called()

    @pytest.mark.asyncio
    async def test_authenticate_sso_form_not_on_workday(
        self, mock_page: MagicMock, config: WorkdayConfig, console: Console
    ) -> None:
        """Test authentication fails when SSO form appears but redirect fails."""
        # Simulate SSO form appearing but staying on auth page
        mock_page.url = "https://auth.example.org/sso"
        login_form = MagicMock()
        login_form.wait_for = AsyncMock(side_effect=Exception("Form appeared"))
        mock_page.get_by_role.return_value = login_form
